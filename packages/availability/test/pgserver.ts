...
const PG_BIN =
  ...
  ''
const PG_DIR = join(tmpdir(), 'plunj-pg-availability')
const DATA_DIR = join(PG_DIR, 'data')
const PG_PORT = 54329
const PG_USER = 'plunj'
const PG_DATABASE = 'plunj_availability_test'
...
export default async function setup(): Promise<() => Promise<void>> {
  ...
  // Prisma 7's `db push` never runs generators, so no --skip-generate needed;
  // packages/db/prisma.config.ts picks the URL up from DATABASE_URL.
